...
<div class="btn-toolbar mb-2 mb-md-0">
    <div>
        <a href="{{ route('kehadiran.show', $absensi->id) }}" class="btn btn-sm btn-light">
...
<div>
    <div class="p-3 rounded bg-light border my-3 d-flex align-items-center justify-content-between">
        ...
        <div>Jumlah : <span class="fw-bold">{{ $izins->count() }}</span></div>
    </div>
    @if (count($izins) === 0)
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <button class="badge text-bg-info border-0 permission-detail-modal-triggers"
                            data-permission-id="{{ $izin->id }}" data-bs-toggle="modal"
                            data-bs-target="#permission-detail-modal">Lihat
                            Alasan</button>
                    </td>
                    @else
                    <td>
                        <form action="{{ route('kehadiran.acceptPermission', $absensi->id) }}" method="post">
                            @csrf
                            <input type="hidden" name="user_id" value="{{ $izin->user->id }}">
                            <input type="hidden" name="tgl_izin" value="{{ $izin->tgl_izin }}">
...
@push('script')
<script>
    const permissionUrl = "{{ route('api.izin.show') }}";
</script>
