<div>
    ...
    @if ($absensi->data->is_using_qrcode && !$data['is_there_permission'])

    {{-- jika belum absen dan absen masuk sudah dimulai --}}
    @if ($absensi->data->is_start && !$data['is_has_enter_today'])
    ...
    <a href="{{ route('home.permission', $absensi->id) }}"
    ...
    @if ($absensi->data->is_end && $data['is_has_enter_today'] && $data['is_not_out_yet'])
    ...
    @if ($data['is_has_enter_today'] && !$absensi->data->is_end)
